...
**§12 is half done.** The feature services — registration, login, logout, users, MFA, passkeys, social, API keys, audit, email — do not exist, so 42 of 43 controller actions still return `501` via `ApiControllerBase.NotImplementedYet()`. Only `/.well-known/jwks.json` is live. Also missing: the cleanup background worker and `Documentation/Architecture/Services.md`. Do not write feature code ahead of the workstream that owns it.
