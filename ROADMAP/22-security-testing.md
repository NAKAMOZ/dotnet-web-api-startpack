...
- [ ] JWT attacks: `alg: none`; HS256-signed token using the public key as HMAC secret (algorithm confusion); tampered payload; expired token (time advance); wrong `iss`/`aud`; unknown `kid`; retired-key `kid`.
...
- [ ] CSRF: cookie-mode state changes without/with-wrong `X-CSRF-Token`; CSRF token bound to session.
