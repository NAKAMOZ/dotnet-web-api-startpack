...
1. **A placeholder authentication scheme was used during the §11 milestone** and was
   removed when the real JWT/API-key policy scheme landed in §12.
   ...
   `FallbackPolicy` is now deny-by-default. It applies to requests matching no endpoint as
   well, so anonymous unknown paths intentionally answer 401.

2. **The milestone initially used explicit `501 Not Implemented` stubs.** §12 replaced all
   41 stubs with feature-service calls and removed the placeholder helpers from
   `ApiControllerBase`.
...
All inventory routes respond; architecture tests green; zero business logic in controllers by review.

**Status: met.** Re-verified after §12:
...
None. Registration now returns a byte-identical `202 Accepted` response for new and
existing addresses; the existing-address path sends the separate registration-attempt
notice.
